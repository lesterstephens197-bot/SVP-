...
uploaded_file = st.sidebar.file_content = st.sidebar.file_uploader("上传 Excel / CSV 文件", type=["xlsx", "xls", "csv"])
...
if uploaded_file is not None:
    ...
    with tab1:
        ...
        sku_col = 'OMS ID'
        name_col = '产品名称' if '产品名称' in df.columns else 'Merchant SKU'
    ...
    with tab2:
        ...
        # 聚合成每个 OMS ID 的 SVP 汇总数据
        group_cols = ['OMS ID']
        for c in ['Merchant SKU', 'Vendor SKU', '产品名称', '品牌', '运营']:
            if c in df.columns:
                group_cols.append(c)

        svp_df = df.groupby(group_cols).agg(
        ...
        # 调整展示列顺序
        display_cols = [c for c in group_cols] + [
            'Actual_YTD_Units', 
            'Annual Sales Units', 
